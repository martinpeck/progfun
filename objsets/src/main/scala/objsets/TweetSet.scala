...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(currentMax: Tweet) = currentMax
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(currentMax: Tweet) : Tweet = 
  {
    left.mostRetweetedAcc(right.mostRetweetedAcc( if (elem.retweets > currentMax.retweets) elem else currentMax))
  }
